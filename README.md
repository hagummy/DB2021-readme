...
이화데벡과(9조)가 만든 프로그램은 사용자가 원하는 영화, 배우, 감독에 대한 정보를 쉽게 검색할 수 있는 프로그램이다. 관리자는 새로운 정보를 삽입하거나, 기존에 있던 정보를 삭제 및 업데이트할 때 불편함 없이 입력할 수 있도록 구현하였다.
...
이화데벡과(9조) 프로그램은 '자바 콘솔 창'에서 실행이 되게 구현하였다.

JAVA 프로젝트 명 : DB2021Team09
...
만약, 프로그램이 돌아가지않는다면, 위 항목 중 잘못된 곳이 있는 지 확인해본다.

SQL에 모든 데이터를 넣는다. 

**한가지 주의해야할 점이 있다면, 모든 데이터를 입력 후, 꼭!  창을 껐다 켜야 한다. 그래야 한글 데이터가 잘 출력이 된다.**

응용 프로그램 상에서는 껐다 키지 않아도 잘 돌아간다. 

JAVA 응용 프로그램 Eclipse를 실행시킨 후, 'DB2021Team09' 파일을 연 후 'RUN' 버튼을 누르면 콘솔 창에서 원하는 작업을 진행할 수 있다. 
...
(2) __배우삭제__ 에는 시리얼 번호로 삭제, 이름으로 삭제가 있다.
